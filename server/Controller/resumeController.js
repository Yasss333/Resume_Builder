...
export const createResumeHandler=async(req,res)=>{
    ...
    try {
        ...
        return res.status(200).json({message:"Succesfully created your resume",
            resume:resume
        });

    } catch (error) {
        return res.status(400).json({message:"Failed to create the resume ", error:error.message})
    }
}
...
export const deleteResumehandler=async(req,res)=>{
    try {
        ...
        return res.status(400).json({message:"Failed to delete the resume ", error:error.message})
    }
...
export const getResumeById=async(req,res)=>{
    ...
    try {
        ...
        return res.status(200).json({message:"Resume diplayed Succesfully ",resume});
        
    } catch (error) {
        
        return res.status(400).json({message:"Failed to get the resume ", error:error.message})
    }
...
export const getPublicResumeById=async(req,res)=>{
    try {
        ...
        if(!resume){
            return res.status(401).json({message:"Either resume not public or no such hresume "})
        }
        
        return res.status(200).json({resume})
    } catch (error) {
        return res.status(400).json({message:"Failed to get the this public resume ", error:error.message})
...
export const updateResumeHandler=async(req,res)=>{
    try {
        ...
        const resume =await Resume.findByIdAndUpdate({userId,_id:resumeId},resumecpy,{new:true})
